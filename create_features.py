...
class Quantizer(object): 
    ...
    def get_feature_vector(self, img, kmeans, centroids): 
        ...
        if fvs is None:
            # Si no hay descriptores, devuelve un vector vacío del tipo correcto
            return np.zeros((1, len(centroids)), dtype=np.float32)

        # 🔧 Asegurar tipo correcto
        fvs = np.asarray(fvs, dtype=np.float32)

        # ✅ Fuerza el tipo antes de pasar a KMeans (esto es lo más importante)
        labels = kmeans.predict(fvs.astype(np.float32))

        # 🔧 También aquí, fuerza el tipo a float32
        fv = np.zeros(len(centroids), dtype=np.float32)
        ...
        # 🔧 Normaliza y vuelve a convertir a float32 por si la normalización cambia el tipo
        fv_image = self.normalize(fv_image).astype(np.float32)
